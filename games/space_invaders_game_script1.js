...
// Touch control for game restart
function handleTouchRestart() {
    if (gameState !== "playing") {
        // Add touch event to restart game
...
function update() {
    ...
    if (gameState === "playing") {
        ...
        if (respawnTimer <= 0) {
            ...
            // --- Player Firing ---
            if ((keys[" "] || touchControls.fire) && fireCooldown <= 0) {
                playerBullets.push({
...
function draw() {
    ...
    if (gameState !== "playing") {
        ...
        ctx.fillText("Press Enter or Touch", GAME_WIDTH / 2, GAME_HEIGHT / 2 + 10);
        ctx.fillText("to Restart", GAME_WIDTH / 2, GAME_HEIGHT / 2 + 16);

        // Handle touch restart
        handleTouchRestart();
    }
}
...
window.onload = function () {
    ...
    document.addEventListener('overscroll', (e) => e.preventDefault());
